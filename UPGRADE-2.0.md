...
As you have a fresh install of the PIM 2.0 containing only one bundle on it (Akeneo/Bundle/MigrationBundle), you can decide to use it or create your own
bundle to put your Custom Code into it.

You need to add your custom config stored in `app/config/config.yml`, `app/config/pim_parameters.yml` into the new PIM.
You also need to add your own dependencies (front and back) into the PIM and activates them in the kernel `app/AppKernel.php`  
...
1. Run `php bin/console pim:installer:dump-require-paths`
2. Run `php bin/console assets:install --symlink`
3. Run `yarn install` inside your project root
4. Run `yarn run webpack`
5. Run `php bin/console cache:clear`
6. Add to your .gitignore the following lines:
